...
mov r1, #0 			//Initalize the sum of r1 as 0

Loop:
ldr    r0, [r2], #4 // Read address with post-increment (r0 = *r2, r2 += 4)
add    r1, r1, r0   // Sum the value of r0 with r1, and store in r1
cmp    r3, r2       // Compare current address with end of list
bne    Loop         // If not at end, continue

lsl r5, r6, #4  // Calculates the total bytes by using a left shift
				// r5 = r4 * 4
